...
mod mint_artwork {
	...
	fn test_transaction_is_ok() {
		...
		let (test_validator, fee_payer_info) = TestValidatorGenesis::default()
			.add_program("digital_art_marketplace", program_id)
			.start();
		let rpc_client = test_validator.get_rpc_client();

		let new_artwork_seed_creator: Pubkey = Pubkey::new_unique();

		let (new_artwork_pubkey, _) = Pubkey::find_program_address(
			&[b"artwork", new_artwork_seed_creator.as_ref()],
			&program_id,
		);
		let accounts = vec![
			AccountMeta::new(fee_payer_info.pubkey(), true),
			AccountMeta::new(new_artwork_pubkey, false),
			AccountMeta::new_readonly(system_program::id(), false),
		];
		let data = DigitalArtMarketplaceInstruction::MintArtwork(crate::generated::instructions::MintArtworkArgs{
			new_artwork_seed_creator,
		});
		let signers = vec![
			&fee_payer_info,
        ];
